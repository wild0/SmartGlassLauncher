 ...
 * for the Launcher.
 */
public class SGLauncherModel extends BroadcastReceiver {
    static final boolean DEBUG_LOADERS = false;
    static final String TAG = "Launcher.Model";

    private final SGLauncherApplication mApp;
    ...
    SGLauncherModel(SGLauncherApplication app) {
        mApp = app;
    }
    ...
    static void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        values.put(SGLauncherSetting.Favorites.CONTAINER, item.container);
        values.put(SGLauncherSetting.Favorites.CELLX, item.cellX);
        values.put(SGLauncherSetting.Favorites.CELLY, item.cellY);
        values.put(SGLauncherSetting.Favorites.SCREEN, item.screen);

        cr.update(SGLauncherSetting.Favorites.getContentUri(item.id, false), values, null, null);
    }
    ...
    static boolean shortcutExists(Context context, String title, Intent intent) {
        final ContentResolver cr = context.getContentResolver();
        Cursor c = cr.query(SGLauncherSetting.Favorites.CONTENT_URI,
    ...
    FolderInfo getFolderById(Context context, HashMap<Long, FolderInfo> folderList, long id) {
        final ContentResolver cr = context.getContentResolver();
        Cursor c = cr.query(SGLauncherSetting.Favorites.CONTENT_URI, null,
                "_id=? and (itemType=? or itemType=?)",
                new String[]{String.valueOf(id),
                        String.valueOf(SGLauncherSetting.Favorites.ITEM_TYPE_USER_FOLDER),
                        String.valueOf(SGLauncherSetting.Favorites.ITEM_TYPE_LIVE_FOLDER)}, null);

        try {
            if (c.moveToFirst()) {
                final int itemTypeIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.ITEM_TYPE);
                final int titleIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.TITLE);
                final int containerIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.CONTAINER);
                final int screenIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.SCREEN);
                final int cellXIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.CELLX);
                final int cellYIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.CELLY);

                FolderInfo folderInfo = null;
                switch (c.getInt(itemTypeIndex)) {
                    case SGLauncherSetting.Favorites.ITEM_TYPE_USER_FOLDER:
                        folderInfo = findOrMakeUserFolder(folderList, id);
                        break;
                    case SGLauncherSetting.Favorites.ITEM_TYPE_LIVE_FOLDER:
    ...
    static void addItemToDatabase(Context context, ItemInfo item, long container,
        ...
        Uri result = cr.insert(notify ? SGLauncherSetting.Favorites.CONTENT_URI :
                SGLauncherSetting.Favorites.CONTENT_URI_NO_NOTIFICATION, values);
    ...
    static void deleteItemFromDatabase(Context context, ItemInfo item) {
        ...
        cr.delete(SGLauncherSetting.Favorites.getContentUri(item.id, false), null, null);
    }
    ...
    static void deleteUserFolderContentsFromDatabase(Context context, UserFolderInfo info) {
        ...
        cr.delete(SGLauncherSetting.Favorites.getContentUri(info.id, false), null, null);
        cr.delete(SGLauncherSetting.Favorites.CONTENT_URI,
                SGLauncherSetting.Favorites.CONTAINER + "=" + info.id, null);
    }
    ...
    public class Loader {
        ...
        final ArrayList<SGLauncherAppWidgetInfo> mAppWidgets = new ArrayList<SGLauncherAppWidgetInfo>();
        ...
        private class LoaderThread extends Thread {
            ...
            private void loadWorkspace() {
                ...
                */
                ...
                final Cursor c = contentResolver.query(
                        SGLauncherSetting.Favorites.CONTENT_URI, null, null, null, null);

                try {
                    final int idIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites._ID);
                    final int intentIndex = c.getColumnIndexOrThrow
                            (SGLauncherSetting.Favorites.INTENT);
                    final int titleIndex = c.getColumnIndexOrThrow
                            (SGLauncherSetting.Favorites.TITLE);
                    final int iconTypeIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.ICON_TYPE);
                    final int iconIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.ICON);
                    final int iconPackageIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.ICON_PACKAGE);
                    final int iconResourceIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.ICON_RESOURCE);
                    final int containerIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.CONTAINER);
                    final int itemTypeIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.ITEM_TYPE);
                    final int appWidgetIdIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.APPWIDGET_ID);
                    final int screenIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.SCREEN);
                    final int cellXIndex = c.getColumnIndexOrThrow
                            (SGLauncherSetting.Favorites.CELLX);
                    final int cellYIndex = c.getColumnIndexOrThrow
                            (SGLauncherSetting.Favorites.CELLY);
                    final int spanXIndex = c.getColumnIndexOrThrow
                            (SGLauncherSetting.Favorites.SPANX);
                    final int spanYIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.SPANY);
                    final int uriIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.URI);
                    final int displayModeIndex = c.getColumnIndexOrThrow(
                            SGLauncherSetting.Favorites.DISPLAY_MODE);

                    ApplicationInfo info;
                    String intentDescription;
                    Widget widgetInfo;
                    SGLauncherAppWidgetInfo appWidgetInfo;
                    int container;
                    long id;
                    Intent intent;
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            switch (itemType) {
                                case SGLauncherSetting.Favorites.ITEM_TYPE_APPLICATION:
                                case SGLauncherSetting.Favorites.ITEM_TYPE_SHORTCUT:
                                    ...
                                    if (itemType == SGLauncherSetting.Favorites.ITEM_TYPE_APPLICATION) {
                                    ...
                                    if (info != null) {
                                        if (itemType
                                                != SGLauncherSetting.Favorites.ITEM_TYPE_APPLICATION) {
                                        ...
                                        switch (container) {
                                            case SGLauncherSetting.Favorites.CONTAINER_DESKTOP:
                                                mItems.add(info);
                                                break;
                                            default:
                                ...
                                case SGLauncherSetting.Favorites.ITEM_TYPE_WIDGET_SEARCH:
                                    widgetInfo = Widget.makeSearch();

                                    container = c.getInt(containerIndex);
                                    if (container != SGLauncherSetting.Favorites.CONTAINER_DESKTOP) {
                                ...
                                case SGLauncherSetting.Favorites.ITEM_TYPE_APPWIDGET:
                                    // Read all Launcher-specific widget details
                                    int appWidgetId = c.getInt(appWidgetIdIndex);
                                    appWidgetInfo = new SGLauncherAppWidgetInfo(appWidgetId);
            ...
            private void bindWorkspace() {
                ...
                for (int i = 0; i < N; i++) {
                    final SGLauncherAppWidgetInfo widget = mAppWidgets.get(i);
                ...
                mHandler.post(new Runnable() {
                    public void run() {
                        ...
                        if (SGLauncherActivity.PROFILE_ROTATE) {
    ...
    private static ApplicationInfo getApplicationInfo(PackageManager manager, Intent intent,
        ...
        info.itemType = SGLauncherSetting.Favorites.ITEM_TYPE_APPLICATION;
        return info;
    }
    ...
    private static ApplicationInfo getApplicationInfoShortcut(Cursor c, Context context,
        ...
        final ApplicationInfo info = new ApplicationInfo();
        info.itemType = SGLauncherSetting.Favorites.ITEM_TYPE_SHORTCUT;

        int iconType = c.getInt(iconTypeIndex);
        switch (iconType) {
            case SGLauncherSetting.Favorites.ICON_TYPE_RESOURCE:
                ...
                break;
            case SGLauncherSetting.Favorites.ICON_TYPE_BITMAP:
    ...
    private static void updateShortcutLabels(ContentResolver resolver, PackageManager manager) {
        final Cursor c = resolver.query(SGLauncherSetting.Favorites.CONTENT_URI,
                new String[]{SGLauncherSetting.Favorites._ID, SGLauncherSetting.Favorites.TITLE,
                        SGLauncherSetting.Favorites.INTENT, SGLauncherSetting.Favorites.ITEM_TYPE},
                null, null, null);

        final int idIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites._ID);
        final int intentIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.INTENT);
        final int itemTypeIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.ITEM_TYPE);
        final int titleIndex = c.getColumnIndexOrThrow(SGLauncherSetting.Favorites.TITLE);
                        ...
                        if (Intent.ACTION_MAIN.equals(shortcut.getAction())) {
                            ...
                            if (name != null) {
                                ...
                                if (title == null || !title.equals(label)) {
                                    final ContentValues values = new ContentValues();
                                    values.put(SGLauncherSetting.Favorites.TITLE, label);

                                    resolver.update(
                                            SGLauncherSetting.Favorites.CONTENT_URI_NO_NOTIFICATION,
                                            values, "_id=?",
